...
\subsection{Gaia DR3}
The stellar system consists of a binary with an M dwarf companion HAT-P-67B separated on-sky by $9\farcs0$ \citep{2019MNRAS.490.5088M}.  The IAU naming convention would demand HAT-P-67A\emph{b} to refer to the planet.  HAT-P-67A (\emph{Gaia DR3 1358614983131339392}) has a parallax of $2.69\pm0.01$ mas in \emph{Gaia} DR3, placing it at about 372$\pm$1.4 pc, slightly farther than previously estimated by \citet{2017AJ....153..211Z} using \emph{Gaia} DR1. Its coarse \emph{Gaia}-based estimates for RV and Doppler broadening are consistent with the more precise values published in \citet{2017AJ....153..211Z}.  Its wide companion HAT-P-67B (\emph{Gaia DR3 1358614983131339904}) \citep{2019MNRAS.490.5088M} has statistically identical parallax ($2.58\pm0.05$ mas) and proper motions, confirming its interpretation as co-moving, with projected separation of about 3400 AU.  Hereafter, we simply drop the A designation for notational simplicity, since the wide companion will not factor into our analysis.
...
% We will probably delete all of this in favor of fitting just the static-subtracted strategy, and merely stating the Si internal precision.
We estimated bulk characteristics of the spectral line by fitting a pair of Voigt profiles to the 10828$-$10838 \AA region.  The blue Voigt profile was intended to fit the Si line centered at 10830 \AA, acting as static control sample.  The red line initialized at 10833 \AA was intended to aglomerate all three components of the metastable triplet into a single feature, consistent with their appearance in Figure \ref{fig:HPFheliumOverview}.  The model constructed in this way has a total of 8 parameters: amplitude $A$, location $\lambda_c$, Gaussian width $\sigma$, and Lorentzian width $\gamma$ for each of the two pseudo-lines.  This phenomenological choice was intended not to match the underlying substructure of the line, but instead merely to match the bulk characteristics in the region of interest.  The choice of Voigt profiles---as opposed to simple Gaussians---provides enough model flexibility to make up for possible deficiencies in the continuum assignment and line shapes.

The individual fits to the Si line show a line center position of typically $X\pm z$.  The weighted mean of all 152 spectra shows a mean of $Y\pm w$.  In contrast, the Helium 10833 bulk feature shows a phase-dependent velocity shift.  The out-of-transit phases exhibit \censor{XX}, consistent with the stellar \ion{He}{1} rest frame zero velocity.  The extended ingress and in-transit phases exhibit a blueshift of \censor{xpmz}.

The model constructed in this way smears the apparent shift, since the static underlying stellar spectrum tends to dilute the dynamic portion of the line.  So we repeated the line fitting by subtracting off a model of the out-of-transit stellar spectrum.  While any interpolation strategy would work, we chose to a Gaussian Process GP model \censor{cite DFM}.  This GP strategy has the advantage of providing principled uncertainty bands, useful for quantifying our confidence in putative weak signals in the presence of correlations.
