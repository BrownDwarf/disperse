...
\section{1D Parker wind models and mass loss rates}
...
Equipped with these three SEDs of differing radiation hardness, we explored different mass loss rates and exosphere temperatures.  Figure \ref{fig:pwinds} shows one example model spectrum with $\dot{M} = 7\times10^{13}$ g/s, and $T_0=9100$~K.  The model spectrum exhibits approximately the same line width and depth as our HPF observations, making it a plausible solution.  The model-dependent line profile (\emph{i.e. width and depth}) is degenerate with XUV flux, $\dot{M}$, and $T_0$.  We were able to replicate the line profile with mass loss rates as high as \censorbox{X}, assuming \censorbox{Y}, or as low as \censorbox{Z} assuming \censorbox{W}.
