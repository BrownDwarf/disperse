...
As mentioned in Section \ref{pwinds}, inferring the mass loss rate from observations is strongly model dependent, as seen in \censorbox{cite e.g. S.V. papers}.  In particular, the uncertain XUV-to-NUV flux ratio introduces a large degeneracy in


the observations indicate a 3D geometry of the outflow-- a preference for escaping material to overtake the planet along the orbital path.  Once the The 1D Parker wind models for high mass loss rates
During the moment of the planet transit midcenter ($\phi=0.0$), the subsolar point on the planet dayside will have material be preferrentially removed from the line-of-sight
