...
Equipped with these two SEDs of differing radiation hardness, we explored different mass loss rates and exosphere temperatures.  Figure \ref{fig:pwinds} shows one example model spectrum with $\dot{M} = 7\times10^{13}$ g/s, and $T_0=9100$~K, with the high XUV spectrum ($L_x/L_\mathrm{bol}=10^{-4}$).  This mass loss rate would imply a characteristic lifetime of $\sim$500 Myr.  The model spectrum exhibits approximately the same line width and depth as our HPF observations, making it a conceivable scenario among a family of partially degnerate solutions.
